...
const NavBar = () => {
  ...
  return (
    <div className={`fixed z-20 top-0 w-screen bg-yellow-400`}>

      {/* HONEY */}
      <svg viewBox="0 0 1440 320" className={`absolute scale-y-40 z-0 ${sticky ? "-mt-60" : "lg:-mt-8 md:-mt-4 -mt-2 "} ease-in-out duration-300`}>
      ...
      {/* DESKTOP NAVBAR */}
      <div className="xl:grid hidden relative z-10 w-5/12 lg:ml-12 ml-4 grid-flow-row grid-cols-4 text-center justify-center items-center">
        <img src={logo} className={`cursor-pointer`} onClick={()=>{window.location.reload()}}></img>
        <div className={`flex px-4 items-center w-full justify-between flex-auto text-center text-yellow-800 font-light-bubble 
                        whitespace-nowrap lg:text-2xl md:text-xl duration-300 font-extrabold col-span-3`}>
